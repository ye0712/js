// @name Emby-ATRCT
// @author ye0712
// @description 直连 Emby 接口，多服务器配置（刮削：不支持，弹幕：不支持，嗅探：不支持）
// @dependencies axios
// @version 1.0.1
// @downloadURL https://github.com/ye0712/js/raw/refs/heads/main/emby-atrct.js

/**
 * ============================================================================
 * Emby 模板一 - TVBox T4 接口格式（多账号版）
 * ============================================================================
 * 说明：
 * 1. 基于本地调试 Emby.js 的逻辑实现 home/category/search/detail/play。
 * 2. 支持多账号配置，可通过 params.extend 覆盖默认账号。
 * 3. 账号密码自动登录拿 token，无需预配置 token。
 */
...
const defaultAccounts = [
  {
    server: "https://eb.atrct.cn", // Emby 服务器地址
    username: "ye0712",             // Emby 账号
    password: "JxXJejqVzruzKgCytbGrkQ", // Emby 密码
    name: "ATRCT",                  // Emby 服务器名称
  },
];
...
const logInfo = (message, data = null) => {
  ...
  OmniBox.log("info", `[Emby-ATRCT] ${output}`);
};

const logError = (message, error) => {
  OmniBox.log("error", `[Emby-ATRCT] ${message}: ${error?.message || error}`);
};
...
async function home(params = {}) {
  ...
  for (let i = 0; i < accounts.length; i += 1) {
    ...
    try {
      ...
      const data = await requestJson(`${url}?${new URLSearchParams(apiParams)}`, {}, buildHeaders());
...
async function play(params = {}) {
  ...
  try {
    ...
    if (!mediaSource) {
      return { urls: [], parse: 1, header: {}, msg: "无可用播放地址" };
    }

    let playUrl = mediaSource.Path || "";
    if (!playUrl && mediaSource.DirectStreamUrl) {
      playUrl = baseUrl + mediaSource.DirectStreamUrl;
    }
    if (!playUrl) {
      playUrl = baseUrl + "/emby/videos/" + itemId + "/stream?Static=true&MediaSourceId=" + mediaSource.Id + "&DeviceId=" + embyInfos.SessionInfo.DeviceId + "&api_key=" + embyInfos.AccessToken + "&PlaySessionId=" + (result.PlaySessionId || "");
    }

    return {
      urls: [{ name: "EMBY", url: playUrl }],
      parse: 0,
