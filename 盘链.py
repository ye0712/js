...
class Spider(BaseSpider):
    ...
    def init(self, extend=""):
        ...
        self.ua = (
            "Mozilla/5.0 (Linux; Android 16; Pixel 9 Pro Build/BP1A.250305.019) "
            "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.7743.101 Mobile Safari/537.36"
        )
        ...
        self.session.headers.update({
            ...
            "Accept": "*/*",
            "Referer": self.site + "/all-videos.php",
        ...
        if extend:
            try:
                ...
                if isinstance(cfg, dict):
                    ...
                    self.enable_check = (
                        bool(cfg.get("enable_check", self.enable_check))
                        and bool(self.check_api)
                    )
            except Exception:
                pass
        self.session.cookies.set(
            "announcement_dismissed", "true", domain="pinglian.lol", path="/"
        )
        if self.cookie:
    ...
    def _check_links(self, links, disk_type, batch_size=30):
        if not links or not self.check_api:
            return links
        ok = []
        for i in range(0, len(links), batch_size):
            ...
            try:
                d = requests.post(
                    self.check_api,
                    json={"items": [{"disk_type": disk_type, "url": u} for u in b]},
                    headers={
                        "User-Agent": self.ua,
    ...
    def _pan_url(self, x):
        return x.get("url", "") or (
            self.site + "/api/go.php?t=" + x.get("token", "")
            if x.get("token") else ""
        )
    ...
    def _process_disk(self, k, v):
        ...
        valid = (
            raw
            if not self.enable_check or k in {"others", "guangya"}
            else self._check_links(raw, k)
        )
        if not valid:
            return None, None
        s = set(valid)
        ...
        for x in links:
            ...
            if u not in s:
                continue
            pwd = x.get("password", "")
            if pwd and "pwd=" not in u and "password=" not in u:
                u += ("&" if "?" in u else "?") + "pwd=" + str(pwd)
            eps.append(
                self._safe(x.get("title") or v.get("name") or k)
                + "$" + self._b64e(u)
            )
        if not eps:
            return None, None
    ...
    def _fetch_pan_links(self, name, vid):
        try:
            d = self.session.get(
                self.api_pan,
                params={"keyword": name, "vod_id": vid, "_t": int(time.time() * 1000)},
                timeout=20,
            ).json()
    ...
    def _vod(self, x):
        return {
            "vod_id": self._b64e(x),
            "vod_name": x.get("vod_name", ""),
            "vod_pic": x.get("vod_pic", ""),
            "vod_remarks": x.get("vod_remarks", "") or x.get("type_name", ""),
        }

    def homeContent(self, filter):
        return {
            "class": [{"type_name": v, "type_id": k} for k, v in self.channels.items()],
            "list": [],
            "filters": {},
        }
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        return {
            "list": [self._vod(x) for x in r.get("list", [])],
            "page": page,
            "pagecount": r.get("pagecount", 0),
            "limit": 30,
            "total": r.get("total", 0),
        }
    ...
    def detailContent(self, ids):
        ...
        name = x.get("vod_name", "")
        fs, us = [], []
        pf = x.get("vod_play_from", "")
        pu = x.get("vod_play_url", "")
        if pf and pu:
            fs.append(pf)
            us.append(pu)
        pan_f, pan_u = (
            self._fetch_pan_links(name, x.get("vod_id"))
            if name and x.get("vod_id") is not None
            else ("", "")
        )
        if pan_f and pan_u:
            fs.append(pan_f)
            us.append(pan_u)
        if not fs:
            fs, us = ["提示"], ["需要有效登录或暂无资源$noop"]
        return {
            "list": [{
                "vod_id": ids[0],
                "vod_name": name,
                "vod_pic": x.get("vod_pic", ""),
                "vod_year": x.get("vod_year", ""),
                "vod_area": x.get("vod_area", ""),
                "vod_actor": x.get("vod_actor", ""),
                "vod_director": x.get("vod_director", ""),
                "vod_content": x.get("vod_content", ""),
                "vod_remarks": (
                    str(x.get("vod_remarks", "")) + " " + str(x.get("vod_score", ""))
                ).strip(),
    ...
    def _real_pan_url(self, u):
        if not isinstance(u, str) or "api/go.php" not in u:
            return u
        try:
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        pans = [
            ...
            "aliyundrive.com", "alipan.com", "cloud.189.cn",
            "123pan.com", "pan.xunlei.com", "115.com",
        ]
        if any(x in u for x in pans):
            return {
                "parse": 0, "jx": 0,
                "url": "push://" + u,
