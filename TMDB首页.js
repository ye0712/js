...
function getCategoryParams(categoryType) {
    switch (categoryType) {
        case "tv":
            return { without_genres: "16,10763,10767,10762" };
        case "variety":
            return { with_genres: "10764,10767", without_genres: "16" };
        case "animation":
        ...
        try {
            ...
            if (nextEpisode && nextEpisode.air_date) {
                ...
                if (diffDays <= 0) {
                    ...
                    nextDateInfo = `第${nextEpisode.episode_number}集 ${weekday}更新`;
                } else {
...
function mapToVod(item, mediaType) {
    ...
    if (mediaType === "movie") {
        ...
        if (lastEp && lastEp.episode_number) {
            if (totalEps > 0) {
                doubanScore = `已播${lastEp.episode_number}/${totalEps}集`;
            } else {
                doubanScore = `已播${lastEp.episode_number}集`;
            }
        } else if (totalEps > 0) {
            doubanScore = `全${totalEps}集`;
        } else {
            doubanScore = year || "";
        }
    }
